...
for _ in range(20):
    cur.execute("""
        INSERT INTO DimMovie (title, genre, duration, country, release_year)
        VALUES (%s, %s, %s, %s, %s)
    """, (
...
for _ in range(10):
    cur.execute("""
        INSERT INTO DimCinema (cinema_name, city, hall_number)
        VALUES (%s, %s, %s)
    """, (
        random.choice(cinemas),
        fake.city(),
        random.randint(1, 10)
    ))
...
for _ in range(15):
    cur.execute("""
        INSERT INTO DimStaff (full_name, staff_position)
        VALUES (%s, %s)
    """, (
        fake.name(),
